...
export const COURSE_WIP_KEY = "academix_course_wip"

export default function AddCourse() {
  const dispatch = useDispatch()
  const { course, step } = useSelector((state) => state.course)

  // Persist WIP to sessionStorage on every Redux course/step change
  useEffect(() => {
    if (course?._id) {
      try {
        sessionStorage.setItem(COURSE_WIP_KEY, JSON.stringify({ course, step }))
      } catch { /* sessionStorage quota */ }
    }
  }, [course, step])

  // On mount: if Redux is empty (page was refreshed), restore from sessionStorage
  useEffect(() => {
    if (course) return // Redux already has state — nothing to restore
    try {
      const raw = sessionStorage.getItem(COURSE_WIP_KEY)
      if (!raw) return
      const { course: savedCourse, step: savedStep } = JSON.parse(raw)
      if (!savedCourse?._id) return
      dispatch(setCourse(savedCourse))
      dispatch(setStep(savedStep ?? 2))
